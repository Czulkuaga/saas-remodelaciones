...
export default function TenantOrgCompanyFormClient({ initial }: { initial: TenantSettingsDTO }) {
    ...
    const bp = initial.orgBP;
    const isOrganization = bp?.type === "ORGANIZATION";
    ...
    async function submit(formData: FormData) {
        ...
        setMsg({ type: "success", text: "BP principal actualizado correctamente." });
        router.refresh();
            ...
            <div className="px-6 py-4 bg-slate-50 dark:bg-slate-500/5 border-b border-slate-200 dark:border-fuchsia-500/10 flex justify-between items-center">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-sm">
                        {isOrganization ? "Empresa (Entidad Legal)" : "BP Principal"}
                    </h3>
                </div>
            ...
            <div className="p-6">
                {!bp ? (
                    <div className="rounded-xl border border-fuchsia-500/20 bg-fuchsia-500/5 p-4 text-sm text-slate-600 dark:text-slate-300">
                        Aún no existe el BP principal para este tenant.
                        <div className="mt-3 flex items-center justify-end">
                            ...
                            >
                                {pending ? "Creando..." : "Crear BP principal"}
                            </button>
                        </div>
                    </div>
                ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="flex flex-col gap-2 md:col-span-2">
                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                    {isOrganization ? "Razón social" : "Nombre principal"}
                                </label>
                                <input
                                    name="organizationName"
                                    defaultValue={bp.organizationName ?? ""}
                                    ...
                                    placeholder={
                                        isOrganization
                                            ? "Ej: Remodelaciones Esmeralda S.A.S."
                                            : "Ej: César Alejandro Zuluaga"
                                    }
                                />
                            </div>
                            ...
                            <div className="md:col-span-2 flex items-center justify-between rounded-xl border border-slate-200 dark:border-fuchsia-500/10 bg-slate-50 dark:bg-slate-900/30 px-4 py-3">
                                <div>
                                    <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                                        Estado del BP principal
                                    </p>
                                    <p className="text-xs text-slate-500 dark:text-slate-400">
                                        Activo/inactivo para operación y asignaciones.
                                    </p>
                                </div>
